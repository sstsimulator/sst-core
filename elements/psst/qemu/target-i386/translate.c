...
#define tcg_gen_qemu_ld8u(dst, addr, idx) gen_helper_psst_ld8u(dst, addr)
#define tcg_gen_qemu_ld8s(dst, addr, idx) gen_helper_psst_ld8s(dst, addr)
#define tcg_gen_qemu_ld16u(dst, addr, idx) gen_helper_psst_ld16u(dst, addr)
#define tcg_gen_qemu_ld16s(dst, addr, idx) gen_helper_psst_ld16s(dst, addr)
#define tcg_gen_qemu_ld32u(dst, addr, idx) gen_helper_psst_ld32u(dst, addr)
#define tcg_gen_qemu_ld32s(dst, addr, idx) gen_helper_psst_ld32s(dst, addr)
#define tcg_gen_qemu_ld64(dst, addr, idx) gen_helper_psst_ld64(dst, addr)

#define tcg_gen_qemu_st8(data, addr, idx) gen_helper_psst_st8(data, addr)
#define tcg_gen_qemu_st16(data, addr, idx) gen_helper_psst_st16(data, addr)
#define tcg_gen_qemu_st32(data, addr, idx) gen_helper_psst_st32(data, addr)
#define tcg_gen_qemu_st64(data, addr, idx) gen_helper_psst_st64(data, addr)
                                                  ...
                                                  int search_pc)
{
    ...
    for(;;) {
        int64_t helper_psst_instruction(uint64_t, uint64_t);         

        //Use helper function to enqueue instruction.
        TCGv_i64 pc_v = tcg_const_i64((uint64_t)pc_ptr);
        /* The same "dirty hack" that's in gen-icount.h */
        uint64_t *ilen_arg = gen_opparam_ptr + 1; 
        uint64_t old_pc_ptr = pc_ptr;
        TCGv_i64 len_v = tcg_const_i64(0xdeadbeef);
        TCGv_i64 hret = tcg_temp_new_i64();
        tcg_gen_helper64(helper_psst_instruction, hret, pc_v, len_v);
