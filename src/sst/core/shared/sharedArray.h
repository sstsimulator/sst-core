...
public:
    ...
    */
    SharedArray() : SharedObject(), published(false), data(nullptr) {}
...
private:
        ...
        */
        void setSize(size_t size, const T& init_data, verify_type v_type)
        {
            ...
            if ( size > array.size() ) {
                ...
                if ( v_type == FE_VERIFY ) { written.resize(size); }
    ...
    private:
        ...
        public:
            ...
            explicit ChangeSet(const std::string& name) : SharedObjectChangeSet(name), size(0), verify(VERIFY_UNINITIALIZED) {}
