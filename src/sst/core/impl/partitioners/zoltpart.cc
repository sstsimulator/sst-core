...
extern "C" {
static int
sst_zoltan_count_vertices(void* data, int* UNUSED(ierr))
{
    int rank = 0;
    ...
    if ( 0 == rank ) {
        ...
        partOutput->verbose(
            CALL_INFO, 1, 0,
            "SST queried by Zoltan for partition graph vertices, found %" PRIu64 " in partition graph\n",
...
static void
sst_zoltan_get_vertex_list(
    void* data, int UNUSED(sizeGID), int UNUSED(sizeLID), ZOLTAN_ID_PTR globalIDs, ZOLTAN_ID_PTR localIDs,
    int UNUSED(wgt_dim), float* obj_wgts, int* ierr)
{
    ...
    if ( 0 == rank ) {
        partOutput->verbose(
            CALL_INFO, 1, 0,
            "SST is queried by Zoltan for the graph vertex list, traversing graph to add to Zoltan...\n");
...
static void
sst_zoltan_get_num_edges_list(
    void* data, int UNUSED(sizeGID), int UNUSED(sizeLID), int num_obj, ZOLTAN_ID_PTR UNUSED(globalID),
    ZOLTAN_ID_PTR UNUSED(localID), int* numEdges, int* ierr)
{
    ...
    if ( 0 == rank ) {
        partOutput->verbose(
            CALL_INFO, 1, 0,
            "SST queried by Zoltan for the number of edges in the graph, these will be calculated...\n");
        ...
        for ( int i = 0; i < num_obj; ++i ) {
            ...
            for ( map_itr = link_map.begin(); map_itr != link_map.end(); map_itr++ ) {
                if ( map_itr->component[0] == (ComponentId_t)i ) { this_comp_links++; }
            }
...
static void
sst_zoltan_get_edge_list(
    void* data, int UNUSED(sizeGID), int UNUSED(sizeLID), int num_obj, ZOLTAN_ID_PTR UNUSED(globalID),
    ZOLTAN_ID_PTR UNUSED(localID), int* num_edges, ZOLTAN_ID_PTR nborGID, int* nborProc, int UNUSED(wgt_dim),
    float* UNUSED(ewgts), int* ierr)
{
    ...
    if ( 0 == rank ) {
        ...
        if ( num_obj != (int)c_graph->getComponentMap().size() ) {
            ...
            fprintf(
                stderr, "Expected request for %d vertices but got %d\n", (int)c_graph->getComponentMap().size(),
                num_obj);
            exit(1);
        }
        ...
        for ( int i = 0; i < num_obj; ++i ) {
            ...
            for ( map_itr = link_map.begin(); map_itr != link_map.end(); map_itr++ ) {
                if ( this_comp_links < 0 ) {
                    ...
                    fprintf(
                        stderr,
                        "Error: Zoltan partition scheme failed. More links from a component than anticipated.\n");
                }
...
SSTZoltanPartition::SSTZoltanPartition(RankInfo world_size, RankInfo my_rank, int verbosity)
{
    ...
    partOutput->verbose(
        CALL_INFO, 1, 0, "Initializing Zoltan interface on rank %d out of %d\n", rank.rank, rankcount.rank);
...
void
SSTZoltanPartition::performPartition(PartitionGraph* graph)
{

    if ( 0 == rank.rank ) { assert(nullptr != graph); }
    ...
    int zolt_rc = Zoltan_LB_Partition(
        zolt_config,          /* input (all remaining fields are output) */
    ...
    if ( 0 == rank.rank ) {
        ...
        for ( uint32_t i = 1; i < rankcount.rank; ++i ) {
            partOutput->verbose(
                CALL_INFO, 1, 0, "Export to rank %" PRIu32 " (assigned %" PRIu64 " components).\n", i,
                rank_assignments[i]);
        }
    }

    partOutput->verbose(CALL_INFO, 1, 0, "Freeing Zoltan data structures...\n");
    Zoltan_LB_Free_Part(&import_global_ids, &import_local_ids, &import_ranks, &import_part);
    Zoltan_LB_Free_Part(&export_global_ids, &export_local_ids, &export_ranks, &export_part);
