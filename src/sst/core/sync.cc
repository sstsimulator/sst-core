...
public:
    SyncMaster(SyncD* sync, Core::ThreadSafe::Barrier&, TimeConverter* period);
    virtual ~SyncMaster();
    void execute();
    void print(const std::string& header, Output &out) const;
...
SyncMaster::SyncMaster(SyncD* sync, Core::ThreadSafe::Barrier& barrier, TimeConverter* period) :
    Action(),
    sync(sync),
    barrier(barrier),
    period(period),
...
SyncD::SyncD(Core::ThreadSafe::Barrier& barrier) :
    SyncBase(),
    mpiWaitTime(0.0),
    deserializeTime(0.0),
    barrier(barrier)
...
SyncD::~SyncD()
{
    ...
    if ( mpiWaitTime > 0.0 || deserializeTime > 0.0 )
        Output::getDefaultObject().verbose(CALL_INFO, 1, 0, "SyncD mpiWait: %lg sec  deserializeWait:  %lg sec\n", mpiWaitTime, deserializeTime);
}
    
ActivityQueue* SyncD::registerLink(const RankInfo& to_rank, const RankInfo& from_rank, LinkId_t link_id, Link* link)
{
    SyncQueue* queue;
...
SyncD::execute(void)
{
    // Simulation::getSimulation()->getSimulationOutput().output("Entering SyncD::execute()\n");
