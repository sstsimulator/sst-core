...
void
MersenneRNG::generateNextBatch()
{
    index = 0;
    for ( int i = 0; i < 624; ++i ) {
        uint32_t temp = (numbers[i] & 0x80000000) + (numbers[(i + 1) % 624] & 0x7fffffff);
...
uint64_t
MersenneRNG::generateNextUInt64()
{
...
void
MersenneRNG::seed(uint64_t seed)
{
    numbers    = (uint32_t*)malloc(sizeof(uint32_t) * 624);
