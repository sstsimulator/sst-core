...
void Link::send( SimTime_t delay, TimeConverter* tc, Event* event ) {  
    if ( tc == NULL ) {
        Simulation::getSimulation()->getSimulationOutput().fatal(CALL_INFO, Output::DefaultErrorRC, "Cannot send an event on Link with NULL TimeConverter\n");
    }
...
Event* Link::recv() 
{
    ...
    if ( UNLIKELY( type != POLL ) ) {
        Simulation::getSimulation()->getSimulationOutput().fatal(CALL_INFO, Output::DefaultErrorRC, "Cannot call recv on a Link with an event handler installed (non-polling link.\n");
