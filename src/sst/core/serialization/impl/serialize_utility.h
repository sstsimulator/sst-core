...
// member objects are typed offsets within a class, and are portable across runs.                  //
//                                                                                                 //
...
// If it's not an aggregate, it needs to be integer, floating-point, enum, or member object pointer
template <class C, bool = std::is_aggregate_v<C>>
constexpr bool has_no_ptr = std::is_arithmetic_v<C> || std::is_enum_v<C> || std::is_member_object_pointer_v<C>;
...
// bitsets are trivially serializable
template <size_t N>
constexpr bool is_trivially_serializable_v<std::bitset<N>, true> = true;
