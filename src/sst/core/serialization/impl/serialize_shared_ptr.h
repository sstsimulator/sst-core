...
constexpr auto ptr_string =
    [] { static_assert(!is_same_template_v<PTR_TEMPLATE, PTR_TEMPLATE>, "Bad ptr_string template arguments"); };
...
public:
    ...
    void operator()(PTR_TEMPLATE<PTR_TYPE>& ptr, std::shared_ptr<PARENT_TYPE>& parent, serializer& ser, ser_opt_t opt)
    {
        ...
        case serializer::PACK:
        {
            ...
            // If ptr is an expired std::weak_ptr, compare parent to empty pointer for the purposes of this test
    ...
    std::enable_if_t<!std::is_function_v<PTR_TYPE>>>
{
        ...
        serializer& ser, ser_opt_t opt)
    {
        ...
        if constexpr ( !is_unbounded_array_v<PARENT_TYPE> ) {
            ...
            size_t     size = 0;
            if ( mode != serializer::UNPACK ) size = get_array_size(ptr.size, pvt::ptr_string<PTR_TEMPLATE>);
