...
template <template <typename> class PTR_TEMPLATE, bool X = false>
auto ptr_string = [] { static_assert(X != X, "Bad ptr_string template arguments"); };

template <>
inline constexpr char ptr_string<std::weak_ptr>[] = "weak_ptr";

template <>
inline constexpr char ptr_string<std::shared_ptr>[] = "shared_ptr";

template <template <typename> class PTR_TEMPLATE, class PARENT_TYPE, bool X = false>
auto wrapper_string = [] { static_assert(X != X, "Bad wrapper_string template arguments"); };
...
get_shared_ptr_owner_tag(const PTR_TEMPLATE<PTR_TYPE>& ptr, serializer& ser)
{
    ...
    if constexpr ( is_same_template_v<PTR_TEMPLATE, std::weak_ptr> && std::is_array_v<PTR_TYPE> ) {
        // We set weak_ptr on a separate statement so that ptr's borrowed shared_ptr refcount is restored before return
...
template <template <typename> class PTR_TEMPLATE, typename PTR_TYPE, typename PARENT_TYPE>
void
    ...
    const PTR_TEMPLATE<PTR_TYPE>& ptr, const std::shared_ptr<PARENT_TYPE>& parent, size_t* size, serializer& ser)
{
    ...
    if ( nonnull ) {
        ...
        // Compute size of parent object, which depends on a variable size when PARENT_TYPE is an unbounded array type
        size_t parent_size;
        ...
        ptrdiff_t offset = reinterpret_cast<const char*>(addr) - reinterpret_cast<const char*>(parent.get());
...
pack_shared_ptr_parent(const std::shared_ptr<PARENT_TYPE>& parent, size_t* size, serializer& ser, ser_opt_t opt)
{
    ...
    bool nonnull = parent.get() != nullptr;
    ser.primitive(nonnull);

    // If the parent opinter is not null, serialize the parent object
    if ( nonnull ) {
        if constexpr ( is_unbounded_array_v<OWNER_TYPE> ) {
            ...
            if constexpr ( is_trivially_serializable_v<ELEM_TYPE> )
                ...
                serialize_array(ser, parent.get(), opt, *size, serialize_array_element<ELEM_TYPE>);
        }
        else {
            // If the parent owner object is not an unbounded array

            // Serialize the parent owner object
            // TODO: Handle reentrancy by allowing std::shared_ptr objects inside of std::shared_ptr pointees
            SST_SER(*const_cast<OWNER_TYPE*>(reinterpret_cast<const OWNER_TYPE*>(parent.get())), opt);
        }
    }
}
...
public:
    ...
    void operator()(PTR_TEMPLATE<PTR_TYPE>& ptr, std::shared_ptr<PARENT_TYPE>& parent, serializer& ser, ser_opt_t opt)
    {
        ...
        case serializer::PACK:
        {
            ...
            // If ptr is an expired std::weak_ptr, compare empty pointer to parent for the purposes of this test
            if ( ptr.use_count() ? owner_ne(ptr, parent) : owner_ne(PTR_TEMPLATE<PTR_TYPE>(), parent) )
                Output::getDefaultObject().fatal(__LINE__, __FILE__, __func__, 1,
        ...
        if constexpr ( !is_unbounded_array_v<PARENT_TYPE> ) {
            ...
            if ( mode == serializer::SIZER || mode == serializer::PACK ) {
                ...
                if constexpr ( std::is_unsigned_v<SIZE_T> )
                    range_error = ptr.size > SIZE_MAX;
                else if constexpr ( sizeof(SIZE_T) > sizeof(SIZE_MAX) )
                    range_error = ptr.size < 0 || ptr.size > static_cast<SIZE_T>(SIZE_MAX);
                else
                    range_error = ptr.size < 0;
