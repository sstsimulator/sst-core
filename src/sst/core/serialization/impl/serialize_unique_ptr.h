...
//
// Without a SST::Core::Serialization::unique_ptr() wrapper, only a std::unique_ptr with a default deleter and without
// an unbounded array type with runtime size can be supported. It is recast into the more general case handled below.
...
class serialize_impl<pvt::unique_ptr_wrapper<PTR_TYPE, DELETER, SIZE_T>>
{
    ...
    void operator()(pvt::unique_ptr_wrapper<PTR_TYPE, DELETER, SIZE_T>& ptr, serializer& ser, ser_opt_t opt)
    {
        ...
        // Reset the std::unique_ptr
        if ( mode == serializer::UNPACK ) ptr.ptr.reset();
