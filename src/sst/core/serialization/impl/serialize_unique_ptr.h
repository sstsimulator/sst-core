...
// Wrapper class
// Default has a reference to a pointer and a deleter which may be defaulted
...
// For unbounded array types, a reference to a size parameter and a deleter which may be defaulted
...
// Serialize std::unique_ptr with default deleter and no size variable for unbounded arrays
template <class PTR_TYPE>
class serialize_impl<std::unique_ptr<PTR_TYPE, std::default_delete<PTR_TYPE>>,
    std::enable_if_t<!is_unbounded_array_v<PTR_TYPE>>>
{
    void operator()(std::unique_ptr<PTR_TYPE>& ptr, serializer& ser, ser_opt_t opt)
    {
        // Create a wrapper with a default deleter and serialize the wrapper
...
// Serialize std::unique_ptr to types which are unbounded arrays with a runtime size, or with a custom deleter
...
class serialize_impl<pvt::unique_ptr_wrapper<PTR_TYPE, DELETER, SIZE_T>>
{
    ...
    // ELEM_TYPE is the array element type, or PTR_TYPE if PTR_TYPE is not an array
    ...
    void operator()(pvt::unique_ptr_wrapper<PTR_TYPE, DELETER, SIZE_T>& ptr, serializer& ser, ser_opt_t opt)
    {
        ...
        if constexpr ( is_unbounded_array_v<PTR_TYPE> ) {
            ...
            if ( size != 0 ) {
                // Allocate the array
                if ( mode == serializer::UNPACK ) ptr.ptr = std::unique_ptr<OWNER_TYPE>(new ELEM_TYPE[size](), ptr.del);
            ...
            if ( nonnull ) {
                if ( mode == serializer::UNPACK ) {
                    ...
                    if constexpr ( std::is_array_v<OWNER_TYPE> ) {
                        ...
                        ptr.ptr = std::unique_ptr<OWNER_TYPE>(
                            reinterpret_cast<OWNER_TYPE*>(new ELEM_TYPE[std::extent_v<OWNER_TYPE>]()), ptr.del);
                    }
                    else {
                        // For pointers to non-arrays
                        ptr.ptr = std::unique_ptr<OWNER_TYPE>(new OWNER_TYPE(), ptr.del);
                    }
                }
    ...
    friend serialize_impl<std::unique_ptr<PTR_TYPE, DELETER>>;

    SST_FRIEND_SERIALIZE();
};

// Wrapper with unbounded array runtime size
template <class ELEM_TYPE, class DELETER, class SIZE_T>
...
// Wrapper with custom deleter but no unbounded array type with runtime size
template <class PTR_TYPE, class DELETER, class DEL>
std::enable_if_t<!is_unbounded_array_v<PTR_TYPE> && std::is_convertible_v<DEL&&, DELETER>,
...
// Wrapper with custom deleter and unbounded array runtime size
template <class ELEM_TYPE, class DELETER, class SIZE_T, class DEL>
std::enable_if_t<std::is_integral_v<SIZE_T> && std::is_convertible_v<DEL&&, DELETER>,
    pvt::unique_ptr_wrapper<ELEM_TYPE[], DELETER, SIZE_T>>
unique_ptr(std::unique_ptr<ELEM_TYPE[], DELETER>& ptr, DEL&& del, SIZE_T& size)
{
