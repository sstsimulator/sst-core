...
class ser_shared_ptr_packer
{
    ...
    //
    // The std::owner_less<void> functor compares two std::shared_ptr or std::weak_ptr instances with .owner_before() to
    // form an ordering which std::map uses to detect whether two std::shared_ptr or std::weak_ptr pointers have the
    // same owner.
    //
    // The keys are of type std::weak_ptr<const void> because any std::shared_ptr or std::weak_ptr can be converted to a
    // std::weak_ptr<const void> and its ownership can be compared with any other std::shared_ptr or std::weak_ptr.

    std::map<std::weak_ptr<const void>, size_t, std::owner_less<void>> shared_ptr_map;
...
class ser_shared_ptr_unpacker
{
    ...
    //
    // This array keeps an copy of the std::shared_ptr<PARENT_TYPE> owner converted to std::shared_ptr<void> around for
    // every deserialization of std::shared_ptr or std::weak_ptr with the same ownership tag.
    //
    // std::deque is used because it never moves std::shared_ptr around after they are inserted, unlike std::vector.
