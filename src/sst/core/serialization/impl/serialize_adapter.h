...
class serialize_impl<T, std::enable_if_t<is_adapter_v<T>>>
{
    ...
    void operator()(T& v, serializer& ser, ser_opt_t options)
    {
        ...
        case serializer::MAP:
            ...
            SST_SER_NAME(static_cast<S&>(v).c, "container", options); // serialize the underlying container
...
template <typename T>
class serialize_impl<T, std::enable_if_t<std::is_pointer_v<T> && is_adapter_v<std::remove_pointer_t<T>>>>
{
    void operator()(T& obj, serializer& ser, ser_opt_t options)
    {
        if ( ser.mode() == serializer::UNPACK ) obj = new std::remove_pointer_t<T>;
        SST_SER(*obj, options);
    }
    SST_FRIEND_SERIALIZE();
};
