...
SSTJSONModelDefinition::discoverLinks(const json& jFile)
{
    ...
    for ( auto& linkArray : jFile["links"] ) {
        ...
        LinkId_t LinkID = graph->createLink(Name.c_str(), nullptr);
        if ( NoCut ) graph->setLinkNoCut(LinkID);
        ...
        for ( int i = 0; i < 2; ++i ) {
            ...
            if ( i == 1 && NonLocal ) {
                ...
                // -- Rank
                auto item = side->find("rank");
                if ( item != side->end() ) {
                    rank = item.value();
                } else {
                    output->fatal(CALL_INFO, 1,
                        "Error finding rank field of nonlocal right link component for Link=%s from script: %s\n",
                        Name.c_str(), scriptName.c_str());
                }

                // -- Thread
                item = side->find("thread");
                if ( item != side->end() ) {
                    thread = item.value();
                } else {
                    output->fatal(CALL_INFO, 1,
                        "Error finding thread field of nonlocal right link component for Link=%s from script: %s\n",
                        Name.c_str(), scriptName.c_str());
                }

                graph->addNonLocalLink(LinkID, rank, thread);
            } else {
                // Left side or local right
                ...
                CompID = findComponentIdByName(Comp[i]);
                graph->addLink(CompID, LinkID, Port[i].c_str(), Latency[i].c_str());
            }
        }
    }
}
