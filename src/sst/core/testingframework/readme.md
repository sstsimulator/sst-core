...
There is no guarantee on the order of testsuites being run, however, all tests within a testsuite will be run before the next testsuite is started (see concurrent testing for exceptions).  <br/>Tests within a test suite are also not guaranteed to run in a specific order.
...
## **Configuration settings when developing SST Testing Frameworks components**
   * If development of the SST Testing Frameworks components is desired, the developer must configure the SST-Core with the `--enable-testframework-dev` configuration flag and then (re)build and install SST-Core.
      * This will create symbolic links to the SST Testing Frameworks components (instead of copying them), thereby allowing the developer to modify the scripts and test changes without having to re-copy the files to the SST-Core install directory.
...
## **Running SST Testing Frameworks**
  ...
  * Command line arguments:
     ...
     * `-k` = Keep output.  Normally, output directory is deleted before run to ensure clean results, setting this flag will prevent delete of output directory; `[False]`
     * `-c xx` = Run tests concurrently in xx threads; `[8]`  (SEE Concurrent Testing BELOW)
     * `-l` List discovered testscipts instead of running tests; `[False]`
...
## **Testsuite File Naming**
   * Testsuite files typically live in the `tests` subdirectory under core and elements and are named `testsuite_<testtype>_<testsuitename>`
      ...
      * `<testtype>` is a catagory of test types.  A test suites with a type name of `"default"` will be run under normal conditions, all other type names will be run only if discovered (see below).  This allows the user to create specialized test suites that are not normally run, but perform other specialized test functions.
...
Under normal operation, testsuites are automatically discovered (from registered elements). The user can also control testsuite discovery using various command line arguments as described below. <br/>**NOTE: Any discovered testsuites will be run after discovery.**.
  ...
  * At startup of the Testing Frameworks, the `-p` and (`-y` or `-w`) arguments are used to create a list of testsuites to be run.
     * `-p` (--list_of_paths)` argument:
        * If the `-p` argument is empty (by default), test directory paths found in the sstsimulator.conf file (located in the <sstcore_install>/etc directory) will be searched for specific testsuites as described below in "searching for testsuites by type or wildcard".  **NOTE: This is the default operation of the Test Frameworks.**
        ...
        * If the `-p` argument includes a directory (containing 1 or more testsuites), that directory will be searched for specific testsuites as described below.  Muliple directories can be defined.
            * Example: `sst-test-elements -p <PathToMerlinTests> <PathToSambaTests>`

  * Searching for testsuites by type or wildcard:
     * For each test directory identified by the -p argument (or automaticially), the test frameworks will search for testsuites based upon the -y or -w (mutually exclusive) argument as follows:
         * `-y` **AND** `-w` Not Specified:
         ...
         * `-y = <type_name>` :
            * Files named `testsuite_<type_name>_*.py` will be added to the list of testsuites to be run when <type_name> is specifed.  This will run user selected set of testsuites in the directory.
...
## **Running Tests Concurrently**
   ...
   * This requires a 3rd party python module called `testtools`
      ...
      * If `testtools` is not installed, concurrent testing is not available and an error will be indicated.
...
## **Creating New Testsuites**
  ...
  * **SST-Core Tests**
     ...
     * Testscripts that run the SDL Testfiles are located in `<sstcore_source>/src/sst/core/tests`
 ...
 * **SST-Elements and 3rd Party Tests**
     ...
     * Reference Ouput file for the SDL Tests files are typicallylocated in `<element source dir>/src/sst/elements/<element>/tests/refFiles`
     * Testscripts that run the SDL Testfiles are typically located in `<element source dir>/src/sst/elements/<element>/tests`
