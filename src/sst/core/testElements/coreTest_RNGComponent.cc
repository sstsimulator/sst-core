    ...
    Component(id)
{
    ...
    if ( rngType == "mersenne" ) {
        ...
        if ( m_w == 0 || m_z == 0 ) {
            ...
            output->verbose(
                CALL_INFO, 1, 0, "Using Marsaglia Generator with seeds: Z=%" PRIu32 ", W=%" PRIu32 "\n", m_w, m_z);
    ...
    // set our clock
    registerClock("1GHz", new Clock::Handler<coreTestRNGComponent, &coreTestRNGComponent::tick>(this));
}
