  ...
  Component(id)
{
    ...
    remote_component = configureLink( "remoteComponent",
                            new Event::Handler<coreTestMessageGeneratorComponent>(this,
                            &coreTestMessageGeneratorComponent::handleEvent) );

    assert(remote_component);

    //set our clock
    registerClock( clock_frequency_str,
        new Clock::Handler<coreTestMessageGeneratorComponent>(this,
                            &coreTestMessageGeneratorComponent::tick ) );
}
...
void coreTestMessageGeneratorComponent::handleEvent(Event *event)
{
