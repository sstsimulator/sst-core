...
void
JSONConfigGraphOutput::generateJSON(const std::string& indent, const ConfigComponent* comp,
                                    const ConfigLinkMap_t& linkMap) const {

    int num = 0;
    std::string temp
        = indent + "\"name\" : \"" + comp->name + "\",\n" + indent + "\"type\" : \"" + comp->type + "\",\n";
        ...
        for ( auto& pair : comp->enabledStatNames ) {
            ...
            if (num2 > 0) {
                ...
                for ( auto& paramsItr : si->params.getKeys() ) {
                    temp = indent + "\t\t\t{ \"name\" : \"" + paramsItr + "\", \"value\" : \""
                           + si->params.find<std::string>(paramsItr) + "\" }";
                    fprintf(outputFile, "%s", temp.c_str());
                    num2--;
                    (num2 > 0) ? fprintf(outputFile, ",\n") : fprintf(outputFile, "\n");
                }
                fprintf(outputFile, "%s\t\t]\n", indent.c_str());
            }
            fprintf(outputFile, "\n%s\t}", indent.c_str());
    ...
    for (auto& paramsItr : comp->params.getKeys()) {
        temp = indent + "\t{ \"name\" : \"" + paramsItr + "\", \"value\" : \""
               + comp->params.find<std::string>(paramsItr) + "\" }";
        fprintf(outputFile, "%s", temp.c_str());
        num--;
        (num > 0) ? fprintf(outputFile, ",\n") : fprintf(outputFile, "\n");
    }
...
void
JSONConfigGraphOutput::generateJSON(const ConfigLink& link, const ConfigComponentMap_t& compMap) const {
    int minLatIdx = (link.latency[0] <= link.latency[1]) ? 0 : 1;
    std::string temp = "\t\t{\n\t\t\t\"name\" : \"" + link.name + "\",\n" + "\t\t\t\"left\" : \""
                       + compMap[link.component[0]]->name + "\",\n" + "\t\t\t\"leftPort\" : \"" + link.port[0] + "\",\n"
                       + "\t\t\t\"right\" : \"" + compMap[link.component[1]]->name + "\",\n"
                       + "\t\t\t\"rightPort\" : \"" + link.port[1] + "\",\n" + "\t\t\t\"latency\" : \""
                       + link.latency_str[minLatIdx] + "\"\n";
    fprintf(outputFile, "%s", temp.c_str());
}
